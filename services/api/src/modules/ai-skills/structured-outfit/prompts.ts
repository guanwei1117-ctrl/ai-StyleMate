...
- improvements 必须根据模式调整：
  ...
  ${coachMode === 'combination' ? '- 重点给"搭配原理"（颜色/版型/比例）的解释' : ''}`
      : '';

  // 需要裁剪入库时，要求每件衣物输出归一化边界框
  const bboxHint = withBbox
    ? `
6. 每件衣物必须给出 bbox：图片坐标系归一化边界框 [x0, y0, x1, y1]，值 0-1（左上角为原点，x 向右，y 向下）。
   bbox 要尽量贴合该衣物主体，不要包含其他衣物，也不要过大留白。`
    : '';
...
1. items：识别照片中每件衣物，type 从 top / bottom / outerwear / dress / shoes / accessory 中选。
   每件衣物给出名称、颜色、风格标签、适合季节、正式程度(1-5)、百搭程度(1-10)。${bboxHint}
  ...
  "items": [
    {
      ...
      "formality": 3,
      "matchability": 8${withBbox ? ',\n      "bbox": [0.05, 0.1, 0.95, 0.45]' : ''}
    }
  ],
