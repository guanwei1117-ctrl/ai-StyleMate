...
## M16：教练模式行为指引
如果用户信息中包含"教练模式"和"用户穿搭水平"，按以下规则调整：
- 穿搭水平 < 4（新手）：improvements 写成"教基础规则"的形式，每条说明为什么
- 穿搭水平 4-6（基础）：improvements 直接给具体建议，不解释原因
- 穿搭水平 >= 7（高级）：只给关键改进点，不要啰嗦，≤ 3 条即可
- 如果教练模式是"购物顾问"：improvements 末尾加 1 条"下次可补什么单品"
- 如果教练模式是"场合顾问"：improvements 强调"什么场合更合适"而非"颜色怎么改"
- 如果教练模式是"搭配教练"：improvements 解释"为什么这样改"（颜色/版型原理）
- 如果教练模式是"综合顾问"：按高水平处理，简洁直接`;
}
