...
function buildRiskFlags(style: StyleCard, answers: OnboardingAnswers): string[] {
  ...
  if (answers.ageGroup === 'under_18' && style.difficulty >= 5) flags.push('年龄建议：超难风格可能需要较长的搭配经验积累');
  if (answers.ageGroup === '30_plus' && style.difficulty >= 4) flags.push('年龄建议：高难度前卫风格可能与你的年龄段气质有差异');
  return flags;
}
