...
- **过渡期方案（先做这个）**：备案没下来前，先用 `http://服务器公网IP:3000` 直接测试功能（安全组放行 3000/4000，不走 Nginx/域名），备案完成后再切正式域名。见下方「阶段 A」。
...
## 0.5 阶段 A：无域名 IP 直连测试（今天就跑通）

> 目标：不等域名/备案，先让服务器上的完整功能跑起来。

```bash
# 前端构建时 API 地址直接指向服务器 IP 的 4000 端口
export NEXT_PUBLIC_API_URL=http://你的服务器公网IP:4000/api/v1
```

- 阿里云安全组放行入方向：`22`、`3000`、`4000`；
- 全部步骤跑完后，访问 `http://服务器公网IP:3000` 即可使用；
- 域名 + 备案下来后进入「阶段 B」：重新 build 时把 `NEXT_PUBLIC_API_URL` 换成 `https://域名/api/v1`，配 Nginx + SSL，然后关闭安全组的 3000/4000。
...
```bash
cd /opt
git clone https://github.com/guanwei1117-ctrl/cleanfit.git stylemate
cd stylemate
git checkout ai-style-profile-upload   # 当前主开发分支
```

> 私有仓库需要在服务器上配置访问凭据（推荐 Personal Access Token 或 SSH Key）。
> 以后更新代码：`git pull` 后按第 10 节流程重新构建。
